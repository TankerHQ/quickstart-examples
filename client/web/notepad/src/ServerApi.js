const onFailedRequest = async (response) => {
  const text = await response.text();
  throw new Error(`Request failed: (${response.status}): ${text}`);
};

export default class Api {
  constructor(appServerUrl = `http://${window.location.hostname}:8080`) {
    this.appServerUrl = appServerUrl;
  }

  urlFor(path) {
    return `${this.appServerUrl}${path}`;
  }

  async doRequest(path, requestOpts = {}) {
    const response = await this.doRequestUnchecked(path, requestOpts);
    if (!response.ok) {
      await onFailedRequest(response);
    }
    return response;
  }
  ...
  async getUserData(userId) {
    ...
    return onFailedRequest(response);
  }

...
- (void)viewDidLoad {
  ...
  promiseSettledWithMinDelay([[Session sharedSession] tankerReady], 2).then(^{
    [self displayLoginScreen];
  }).catch(^(NSError *error) {
    NSLog(@"%@", error);
    NSString *errorMessage = @"Failed to init the session. Verify that the server is running and restart the application.";

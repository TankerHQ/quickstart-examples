...
PMKPromise<NSArray*>* (^jsonToArray)(NSData*) = ^(NSData* json) {
  NSError* err = nil;
  NSArray* array = [NSJSONSerialization JSONObjectWithData:json options:NSJSONReadingAllowFragments error:&err];
  if (err)
  {
...
PMKPromise<NSDictionary*>* (^jsonToDict)(NSData*) = ^(NSData* json) {
  NSError* err = nil;
  NSDictionary* dict = [NSJSONSerialization JSONObjectWithData:json options:NSJSONReadingAllowFragments error:&err];
  if (err)
  {
    return [PMKPromise promiseWithValue:err];
  }
...
- (PMKPromise<NSDictionary*>*)authenticateWithPath:(NSString*)path email:(NSString*)email password:(NSString*)password
{
...
{
  return dictToJson(@{@"oldPassword" : oldPassword, @"newPassword" : newPassword}).then(^(NSData* jsonBody) {
    return [self.httpClient putWithPath:@"me/password" body:jsonBody contentType:@"application/json"];
  });
}

- (PMKPromise<NSDictionary*>*)resetPasswordTo:(NSString*)newPassword withToken:(NSString*)resetToken
{
...
{
  return dictToJson(@{@"email" : email}).then(^(NSData* jsonBody) {
...
{
  return dictToJson(@{@"from" : self.currentUserId, @"to" : recipients}).then(^(NSData* jsonBody) {
